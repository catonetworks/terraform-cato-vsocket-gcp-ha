locals {
  ...
  effective_load_balancer_ip = coalesce(var.load_balancer_ip, cidrhost(var.subnet_lan_cidr, 6))
  ...
  secondary_serial           = var.ha ? [for s in data.cato_accountSnapshotSite.gcp-site-for-secondary[0].info.sockets : s.serial if s.is_primary == false] : []
